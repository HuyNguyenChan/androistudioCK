...
1. Trong MySQL command line, chạy lệnh:
```sql
source D:/DoAnSinhVien/MusicApp/FresherK8/Server/MusicApp/database_setup.sql
```

**Lưu ý**: Thay đổi đường dẫn cho phù hợp với vị trí file `database_setup.sql` trên máy bạn.

Hoặc nếu bạn đang ở thư mục dự án:
```bash
...
**Nếu MySQL yêu cầu password**, thêm dòng sau vào file `.env`:
```env
DB_PASSWORD=your_password_here
```

Và cập nhật file `database/database.js` để thêm:
```javascript
...
**Quan trọng**: Đảm bảo bạn đã tạo database MySQL trước khi chạy ứng dụng.
...
Server sẽ chạy tại: 
- **Local**: `http://localhost:3000`
- **Network**: `http://YOUR_IP:3000` (IP sẽ được hiển thị khi khởi động)

### Bước 6: Lấy IP address (Cho Android client)

Để Android app kết nối được, bạn cần sử dụng IP của máy tính thay vì `localhost`:
...
**Lưu ý quan trọng**: 
- Android không thể kết nối đến `localhost` hoặc `127.0.0.1`
- Phải dùng IP thực của máy tính (ví dụ: `192.168.1.100`)
- Máy tính và Android device phải cùng mạng WiFi
- Xem file `NETWORK_SETUP.md` để biết chi tiết về cấu hình kết nối Android
...
- Thay đổi port trong `index.js` hoặc
...
Database sau khi tạo sẽ rỗng. Để thêm dữ liệu bài hát:

### Cách 1: Sử dụng file SQL mẫu (Khuyến nghị)

File `sample_data.sql` đã chứa dữ liệu mẫu sẵn:
...
```sql
source D:/DoAnSinhVien/MusicApp/FresherK8/Server/MusicApp/sample_data.sql;
```

### Cách 2: Thêm dữ liệu thủ công

Xem file **`ADD_DATA_GUIDE.md`** để biết hướng dẫn chi tiết về:
- Thứ tự thêm dữ liệu (Category → Topic → Artist → Album → Playlist → Song)
- Cú pháp SQL cho từng bảng
- Cách kiểm tra dữ liệu
- Xử lý lỗi thường gặp

**Lưu ý**: 
- Tất cả URL trong file mẫu là ví dụ, bạn cần thay thế bằng URL thực tế
- Đảm bảo thêm dữ liệu theo đúng thứ tự để tránh lỗi foreign key
